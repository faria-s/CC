...
from lib.structs import RequestMission
...
class MissionLink:
    ...
    def handle_packet_deserialize(self, data: bytes):
        ...
        match packet_type:
            ...
            case PacketType.RequestMission:
                return RequestMission.deserialize(data)
    ...
    def handle_packet(
        ...
        try:
            ...
            if self.__is_server and isinstance(received_packet, RegisterRover):
                ...
                response = connection.handle_sendable_register_response(
                    received_packet, rover_id
                )
                connection.add_packet_to_send(response)

                message = (
                    f"Server confirms connection with {host}:{port} (ID: {rover_id})"
                )

            else:
                connection = self.__connections[host]

                if isinstance(received_packet, EndConnection) or not (
                    connection.running
                ):
                    fynack = connection.handle_sendable_end_connection_response(
                        received_packet
                    )
                    connection.add_packet_to_send(fynack)

                    connection.stop_retransmission_thread()
                    del self.__connections[host]

                    message = "[MISSION LINK] Ending MissionLink Connection with server"

                elif isinstance(received_packet, EndConnectionResponse) or not (
                    connection.running
                ):
                    connection.stop_retransmission_thread()
                    del self.__connections[host]

                    message = f"[MISSION LINK] Ending Connection with {host}:{port}"
                    ...
                    try:
                        ...
                        if (
                            self.__missions_to_send
                            and connection.total_sent_missions < self._max_missions
                        ):
                            mission = self.__missions_to_send.popleft()
                            self.__missions_being_done[host] = mission
                            connection.increment_total_sent_mission()
                            seq, ack = connection.update_seq_ack_number(received_packet)

                            packet = Packet(
                                PacketType.Mission, seq, ack, mission.serialize()
                            )
                        ...
                        message = (
                            f"[ERROR] Received packet from unknown host: {host}:{port}"
                        )

                elif isinstance(received_packet, RequestMission):
                    if self.__missions_to_send:
                        mission = self.__missions_to_send.popleft()
                        self.__missions_being_done[host] = mission
                        seq, ack = connection.update_seq_ack_number(received_packet)
                        ...
                        message = f"Received RequestMission={received_packet.sequence_number} from {host}:{port}; Sending Seq={packet.sequence_number} to {host}:{port}"
                    else:
                        seq, ack = connection.update_seq_ack_number(received_packet)

                        packet = EndConnection(seq, ack)
                        connection.add_packet_to_send(packet)

                        message = f"No more missions to send, Ending Connection with {host}:{port}"
    ...
    def request_mission(self, server_address: tuple[str, int]):
        host, port = server_address
        try:
            connection = self.__connections[host]

            seq = connection.next_sequence_number
            packet = RequestMission(seq, seq + 1)
            connection.add_packet_to_send(packet)

            packets = connection.get_sendable_packets()
            self.send_packets(packets, server_address)
            log("[MISSION LINK] Requesting new Mission...")
        except KeyError:
            log("[MISSION LINK] Server connection closed")
